...
//
// Personal todo list
//     Validate reamer mode ( pretty sure it is bricked )
//     Make it less dependent on the ui and increase pure gcode control.... Long road to go..
//
//############################################################################################################
...
//volatile float wire_speed_mm_min         = 150.0;
//volatile float wire_speed_mm_min_probing = 50.0;
...
volatile float reamer_travel_mm           = DEFAULT_REAMER_TRAVEL;      // the travel distance for the reamer up/down oscillation. 
volatile float reamer_duration            = DEFAULT_REAMER_DURATION;    // the duration of the reasming process in seconds . 0.0 means it runs until manually stopped
volatile float pwm_duty_probing           = DEFAULT_PROBING_DUTY;
volatile int   pwm_frequency_probing      = DEFAULT_PROBING_FREQUENCY;
...
volatile bool  enable_dpm_rxtx            = DEFAULT_ENABLE_DPM;
...
void IRAM_ATTR update_scope_stats(){ // load some static data into the scope
    float duty      = arcgen.get_setting_float( PARAM_ID_DUTY );
    float frequency = float( arcgen.get_setting_int( PARAM_ID_FREQ ) ) / 1000.0;
    gscope.set_pwm_duty( duty );
    gscope.set_pwm_frequency( frequency );
    gscope.set_full_range_size( gsense.get_setting_int( PARAM_ID_FAVG_SIZE ) );
    gscope.set_vdrop_treshhold( gsense.get_setting_int( PARAM_ID_VDROP_THRESH ) ); 
}
...
void GUI::run_once_if_ready(){              // runs once after the ui is ready
    ...
    gsense.generate_setpoint_min_max_adc(); // reset some sensor stuff and set the gscope min max setpoints etc.
...
void GUI::reset_after_job(){
    ...
    int page = PAGE_FRONT;
    if( enable_dpm_rxtx ){ // turn DPM off if support is enabled
...
void GUI::alarm_handler(){
    ...
    if( enable_dpm_rxtx ){ dpm_driver.power_on_off( 0, 3 ); } // turn DPM off
    vTaskDelay(100);
...
void show_credits(){ // create the credits for the boot screen
    if( ! is_machine_state( STATE_REBOOT ) ){
        ...
        const char* credits[] = {
            ...
            "@charlatanshost", "@tommygun", "@Nikolay", "@renyklever", "@Zenitheus", "@gerritv", "@cnc", 
...
void IRAM_ATTR ui_task(void *parameter){
    ...
    if( is_machine_state( STATE_REBOOT ) ){
        ...
        ui.start(); // restores the last session etc., flags for reboot if firmware is freshly installed to write the default settings to the session cache
        
        // now the settings are loaded
        if( ! is_machine_state( STATE_REBOOT ) ){
            if( enable_dpm_rxtx ){
                debuglog( dpm_messages[DPM_WAIT], DEBUG_LOG_DELAY );
                // disabled DPM support if initial connection fails
                enable_dpm_rxtx = dpm_driver.power_on_off( false, 0 ); 
                if( !enable_dpm_rxtx ){
...
void GUI::pre_process_defaults(){
    ...
    gsense.generate_setpoint_min_max_adc();  
    arcgen.pwm_on();
    vTaskDelay(100);
    ...
    vTaskDelay(100);
    return;
    /*int vfd_measured = gsense.get_vfd_feedback(); // pwm should be enabled at this point!
    if( vfd_measured <= gsense.get_setting_int( PARAM_ID_VDROP_THRESH ) ){
        debuglog("*Adjust vFd", DEBUG_LOG_DELAY );
        debuglog("*vDrop threshold (ADC)", DEBUG_LOG_DELAY );
        debuglog("*needs to be smaller then", DEBUG_LOG_DELAY );
        debuglog("*the measured vfd reading", DEBUG_LOG_DELAY );
        vTaskDelay(3000);
    }*/
...
void IRAM_ATTR reamer_task_single_axis(void *parameter){
    ...
    int reamer_axis = axes_active.z.index;
    ...
    float* current_position = system_get_mpos();
    float  backup_position[N_AXIS];
    memcpy( backup_position, current_position, sizeof(current_position[0]) * N_AXIS );

    float travel_left_negative = api::get_max_travel_possible( reamer_axis, 1 );
    ...
    sprintf( command_buf_down, "G90 G1 %s%.5f F30\r\n", get_axis_name_const(reamer_axis), reamer_travel_mm ); 
    sprintf( command_buf_up,   "G90 G1 %s%.5f F30\r\n", get_axis_name_const(reamer_axis), 0.0 ); 

    api::unset_probepositions_for(reamer_axis); // unset the probe position for the sinker axis
    api::auto_set_probepositions();
    ...
    set_system_op_mode( OP_MODE_EDM_REAMER );
    ...
    set_system_op_mode( OP_MODE_NORMAL );
...
void GUI::start_process(void){
  ...
  } /*else if ( operation_mode == MODE_REAMER && axes_active.z.index < N_AXIS ){
      pre_process_defaults();
      xTaskCreatePinnedToCore(reamer_task_single_axis, "reamer_task", 3000, this, 0, &drill_task_handle, 0);
      unsigned long start = millis();
      while( !task_running ){ 
          if( ( millis() - start > 10000 ) || get_quit_motion() ){
            break;
          }
          vTaskDelay(100); 
      }
...
void GUI::probe_done(){
    ...
    if( enable_dpm_rxtx ){ // restore the pre probing dpm settings
...
bool GUI::probe_prepare(){
    ...
    arcgen.probe_mode_on( pwm_frequency_probing, pwm_duty_probing );
    gsense.generate_setpoint_min_max_adc();
...
void set_pwm_freq( setter_param *item ){
    ...
    arcgen.change_pwm_frequency( frequency );
    update_scope_stats();
}
// set the pwm duty; float value in %
void set_pwm_duty( setter_param *item ){
    float pwm_duty = char_to_float( item->second );
    arcgen.update_duty( pwm_duty );
    update_scope_stats();
}
...
void set_pwm_button_state( setter_param *item ){
    ...
    if( state ){
        arcgen.change_setting( PARAM_ID_PWM_STATE, 1 );
    } else {
        arcgen.change_setting( PARAM_ID_PWM_STATE, 0 );
    }
}
...
void set_setpoint_min( setter_param * item ){
    gsense.change_setting( PARAM_ID_SETMIN, 0, char_to_float( item->second ) );
}
// set setpoint max; float value
void set_setpoint_max( setter_param * item ){
    gsense.change_setting( PARAM_ID_SETMAX, 0, char_to_float( item->second ) );
}
// set max short circuit duration; int value
void set_short_duration( setter_param * item ){
    int value = ( char_to_int( item->second ) * 1000 );
    gsense.change_setting( PARAM_ID_SHORT_DURATION, value ); // not used in the sensor but let's keep it
...
void set_average_main( setter_param * item  ){
    int value = char_to_int( item->second );
    if( value < 1 ){ value = 1; } 
    else if( value > vsense_sampler_buffer_size-1 ){ value = vsense_sampler_buffer_size-1; }
    gsense.change_setting( PARAM_ID_MAIN_AVG, value );
}
// set rising boost; int value
void set_rising_boost( setter_param * item  ){
    int value = char_to_int( item->second );
    if( value >= ( vsense_sampler_buffer_size-1 ) ){ value = vsense_sampler_buffer_size-1; } 
    else if( value <= 0 ){ value = 1; }
    gsense.change_setting( PARAM_ID_RISE_BOOST, value );
}
...
void set_samples_pool_size( setter_param * item ){
    int value = MAX( 1, char_to_int( item->second ) );
    gsense.change_setting( PARAM_ID_BEST_OF, value );
}
...
void set_favg_size( setter_param * item ){
    ...
    gsense.change_setting( PARAM_ID_FAVG_SIZE, value );
    update_scope_stats();
}
...
void set_vdrops_block( setter_param * item ){
    gsense.change_setting( PARAM_ID_VFD_DROPS_BLOCK, char_to_int( item->second ) );
}
// set number of vchannel drops to turn pwm off; int value ADC
void set_vdrops_poff( setter_param * item ){
    gsense.change_setting( PARAM_ID_VFD_SHORTS_POFF, char_to_int( item->second ) );
}
...
void set_poff_after( setter_param * item ){
    gsense.change_setting( PARAM_ID_POFF_AFTER, char_to_int( item->second ) );
}
// set use stop/go signals; bool value
void set_use_stopgo_signals( setter_param * item ){
    gsense.change_setting( PARAM_ID_USE_STOPGO, char_to_bool( item->second ) ? 1 : 0 );
}
// set zero jitter; int value
void set_zero_jitter( setter_param * item ){
    gsense.change_setting( PARAM_ID_ZERO_JITTER, MAX( 0, char_to_int( item->second ) ) );
}
// set zero threshold; int value adc
void set_zero_threshold( setter_param * item ){
    gsense.change_setting( PARAM_ID_ZERO_THRESH, char_to_int( item->second ) );
}
// set readings low before it moves on; int value
void set_readings_low( setter_param * item ){
    gsense.change_setting( PARAM_ID_READINGS_L, MAX( 1, char_to_int( item->second ) ) );
}
// set readings high before something happens; int value
void set_readings_high( setter_param * item ){
    gsense.change_setting( PARAM_ID_READINGS_H, MAX( 1, char_to_int( item->second ) ) );
}
...
void set_probe_cfd_trigger( setter_param * item ){
    ...
    gsense.change_setting( PARAM_ID_PROBE_TR_C, 0, value );
}
...
void set_dpm_uart_enabled( setter_param * item ){
    ...
    if( enable_dpm_rxtx && ui.get_is_ready() ){
...
IRAM_ATTR void set_edm_pause( setter_param * item ){
    ...
    if( gconf.edm_pause_motion ){
      ...
      debuglog("@Process paused",DEBUG_LOG_DELAY);
    } else {
...
void get_short_duration( getter_param * item ){
    item->intValue = round( gsense.get_setting_int( PARAM_ID_SHORT_DURATION ) / 1000 );
}
...
void get_average_main( getter_param * item  ){
    item->intValue = gsense.get_setting_int( PARAM_ID_MAIN_AVG );
}
void get_rising_boost( getter_param * item  ){
    item->intValue = gsense.get_setting_int( PARAM_ID_RISE_BOOST );
}
...
void get_samples_pool_size( getter_param * item ){
    item->intValue = gsense.get_setting_int( PARAM_ID_BEST_OF );
}
void get_favg_size( getter_param * item ){
    item->intValue = gsense.get_setting_int( PARAM_ID_FAVG_SIZE );
}
...
void get_vdrops_block( getter_param * item ){
    item->intValue = gsense.get_setting_int( PARAM_ID_VFD_DROPS_BLOCK );
}
void get_vdrops_poff( getter_param * item ){
    item->intValue = gsense.get_setting_int( PARAM_ID_VFD_SHORTS_POFF );
}
...
void get_poff_after( getter_param * item ){
    item->intValue = gsense.get_setting_int( PARAM_ID_POFF_AFTER );
}
void get_zero_jitter( getter_param * item ){
    item->intValue = gsense.get_setting_int( PARAM_ID_ZERO_JITTER );
}
void get_zero_threshold( getter_param * item ){
    item->intValue = gsense.get_setting_int( PARAM_ID_ZERO_THRESH );
}
void get_readings_low( getter_param * item ){
    item->intValue = gsense.get_setting_int( PARAM_ID_READINGS_L );
}
void get_readings_high( getter_param * item ){
    item->intValue = gsense.get_setting_int( PARAM_ID_READINGS_H );
}
...
/*void get_use_hw_timer( getter_param * item ){
    item->boolValue = gsense.get_setting_bool( PARAM_ID_USEHWTIMTER );
}*/
void get_use_stopgo_signals( getter_param * item ){
    item->boolValue = gsense.get_setting_bool( PARAM_ID_USE_STOPGO );
}
void get_setpoint_min( getter_param * item ){
    item->floatValue = gsense.get_setting_float( PARAM_ID_SETMIN );
}
void get_setpoint_max( getter_param * item ){
    item->floatValue = gsense.get_setting_float( PARAM_ID_SETMAX );
}
void get_probe_cfd_trigger( getter_param * item ){
    item->floatValue = gsense.get_setting_float( PARAM_ID_PROBE_TR_C );
}
// set reset sense queue before each step; bool value
void set_reset_vsense_queue( setter_param * item ){
    reset_vsense_queue.store( char_to_bool( item->second ) );
    //plconfig.reset_sense_queue = char_to_bool( item->second );
}
void get_reset_vsense_queue( getter_param * item ){
    item->boolValue = reset_vsense_queue.load();//plconfig.reset_sense_queue;
}
void get_scope_use_high_res(  getter_param * item  ){
    item->boolValue = scope_use_high_res.load();
}
void set_scope_use_high_res(  setter_param * item  ){
    scope_use_high_res.store( char_to_bool( item->second ) );
    gsense.reset_sensor_global();
}
...
void set_motor_res( setter_param * item ){
    ...
    g_axis[axis_index]->steps_per_mm.set( char_to_int( item->second ) );
    api::update_speeds();
}
void get_motor_res( getter_param * item ){
    ...
    item->intValue = g_axis[axis_index]->steps_per_mm.get();
}
...
void GUI::setup_events( void ){
    ...
    const std::function<void(getter_param *item)> get_callback = [](getter_param *item){
        ...
        bool pwm_is_off = arcgen.get_pwm_is_off();
        if( !pwm_is_off ){
            arcgen.pwm_off();
        }
