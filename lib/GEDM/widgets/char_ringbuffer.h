...
private:
    ...
    std::atomic<bool> has_change;       // Indicates new data
    int type_index;                     // Last read index


public:
    CharRingBuffer() : capacity(0), index(0), index_work(0), has_change(false), type_index(0) {
        cringbuffer = nullptr;
        iringbuffer = nullptr;
    }
    ...
    bool has_new_items() const {
        return has_change.load(std::memory_order_acquire);
    }
    ...
    void reset_work_index() {
        ...
        has_change.store(false, std::memory_order_release);
    }
