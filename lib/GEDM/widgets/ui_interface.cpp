...
DRAM_ATTR std::atomic<bool> logger_busy_writing(false);
...
IRAM_ATTR void PhantomUI::reload_page_on_change(){
    ...
    if( 
        reload_waiting.load() ||
        !get_is_ready() || touch.block_touch 
...
void show_logger() { // only used on page creation to enforce a redraw of the logger overlay
    if( !ui.logger->is_visible || !logs_available.load()  || logger_busy_writing.load() ){ return; }
    // Wait until logger_fill is not writing
    acquire_lock_for( ATOMIC_LOCK_LOGGER );
    logs_available.store(false);
    logger_busy_writing.store(true);
    ui.logger->show();
    logger_busy_writing.store(false);
    release_lock_for( ATOMIC_LOCK_LOGGER );
}
...
void charge_logger( const char *line ){
    ...
    logs_available.store(false);
    logger_busy_writing.store(true);
    //ui.logger->show(); // this can dramatically increase stack size if to much data is put in the logger to fast, would be better to call show in the task loop only...
...
// but thos touch events based draws are all happening within this function
//#############################################################################
...
void PhantomUI::set_scope_and_logger( uint8_t page_id ){ // called after the page is drawn
    ...
    if( page_id == PAGE_FRONT ){
        ...
        if( gscope.scope_is_running() ){ gscope.stop(); }
        gscope.init( 320, 75, 0, 0 );
        gscope.start();
    } else {
...
const std::function<void(PhantomWidgetItem *item)> load_pwm_enabled = [](PhantomWidgetItem *item){
    ...
    param_load( item, [](PhantomWidgetItem *item,PhantomSettingsPackage *package){
        ...
        if( ! gscope.get_has_init() ){
            gscope.init( 320, 75, 0, 31 );
            gscope.start();
        }
...
void create_settings_page(){
    ...
    add_std_list_item( settings_8_ptr, OUTPUT_TYPE_INT,    DICT_VDROPS_BLOCK,   PARAM_TYPE_INT,   PARAM_ID_VFD_DROPS_BLOCK, 0 );
    add_std_list_item( settings_8_ptr, OUTPUT_TYPE_INT,    DICT_VDROPS_POFF,    PARAM_TYPE_INT,   PARAM_ID_VFD_SHORTS_POFF, 0 );
    ...
    add_std_list_item_bool( settings_9_ptr, DICT_RESET_QUEUE, PARAM_ID_RESET_QUEUE );
    add_std_list_item( settings_9_ptr, OUTPUT_TYPE_INT,    DICT_BEST_OF,    PARAM_TYPE_INT, PARAM_ID_BEST_OF,     0 );
    ...
    add_std_list_item( settings_9_ptr, OUTPUT_TYPE_INT,    DICT_POFF_AFTER, PARAM_TYPE_INT, PARAM_ID_POFF_AFTER,  0 );
    ...
    add_std_list_title( settings_10_ptr, DICT_STOPGO_PAGE_TITLE );
    add_std_list_item_bool( settings_10_ptr, DICT_USE_STOPGO, PARAM_ID_USE_STOPGO );
    add_std_list_item( settings_10_ptr, OUTPUT_TYPE_INT,    DICT_ZERO_JITTER, PARAM_TYPE_INT, PARAM_ID_ZERO_JITTER, 0 );
    add_std_list_item( settings_10_ptr, OUTPUT_TYPE_INT,    DICT_READINGS_H,  PARAM_TYPE_INT, PARAM_ID_READINGS_H,  0 );
    add_std_list_item( settings_10_ptr, OUTPUT_TYPE_INT,    DICT_EADINGS_L,   PARAM_TYPE_INT, PARAM_ID_READINGS_L,  0 );
    add_std_list_item( settings_10_ptr, OUTPUT_TYPE_STRING, DICT_ZERO_THRESH, PARAM_TYPE_INT, PARAM_ID_ZERO_THRESH, 8 );
    ...
    add_std_list_item( settings_3_ptr, OUTPUT_TYPE_INT, DICT_MAIN_AVG,   PARAM_TYPE_INT, PARAM_ID_MAIN_AVG,   0 );
    add_std_list_item( settings_3_ptr, OUTPUT_TYPE_INT, DICT_RISE_BOOST, PARAM_TYPE_INT, PARAM_ID_RISE_BOOST, 0 );
    add_std_list_item( settings_3_ptr, OUTPUT_TYPE_INT, DICT_FAVG_SIZE,  PARAM_TYPE_INT, PARAM_ID_FAVG_SIZE,  0 );
    ...
    add_std_list_item_bool( settings_4_ptr, DICT_SPINDLE_DIR_INVERTED, PARAM_ID_PULLING_MOTOR_DIR_INVERTED ); // Enable automatic spindle controls
...
void create_motion_page(){
    ...
    for( int i = 0; i < N_AXIS; ++i ){
        ...
        add_std_list_item( settings_12_ptr, OUTPUT_TYPE_INT, text, PARAM_TYPE_INT, param_id, 0 );
    }
...
bool PhantomUI::prepare(){ 
    if( is_machine_state( STATE_REBOOT ) )return true;
    vTaskDelay(DEBUG_LOG_DELAY);
    init_sd();
    return true;
}
...
bool PhantomUI::firmware_is_recent_set_flag(){
    ...
    if( p.begin(RELEASE_NAME, false) ){
        p.putBool( get_firmware_key().c_str(), true );
        p.end();
        return true;
    } return false;
}
...
bool PhantomUI::firmware_is_recent(){
    ...
    if( p.begin(RELEASE_NAME, false) ){
        if( p.getBool( get_firmware_key().c_str() ) ){
            is_recent = true;
        } 
        p.end();
        return is_recent;
...
bool PhantomUI::delete_nvs_storage(){
    ...
    if( p.begin(RELEASE_NAME, false) ){
        if( p.clear() ){
            debuglog("@NVS deleted!");
        } else {
            debuglog("*Failed to delete NVS!");
        }
        p.end();
    } else {
        debuglog("*Failed to open storage ###");
    }
    vTaskDelay(1000);
    return true;
}
...
void PhantomUI::write_calibration_data(){
    ...
    if( p.begin( RELEASE_NAME ) ){
        size_t bytes_write = p.putBytes("calibration", calibration_data, CALDATA_SIZE * sizeof(uint16_t));
        // Check if the read bytes match the expected size
        if (bytes_write == CALDATA_SIZE * sizeof(uint16_t)) {} else {}
    }
    p.end();
}
...
bool PhantomUI::touch_calibrate( bool force ){
    ...
    if( p.begin( RELEASE_NAME ) ){
        ...
        if (force){
            ...
            size_t bytesRead = p.getBytes("calibration", calibration_data, CALDATA_SIZE * sizeof(uint16_t));
            if( bytesRead == CALDATA_SIZE * sizeof(uint16_t) ) {
                debuglog("Calibration data loaded");
                success = 1; // found the data
            } else {}
        }
    } else {
        p.end();
        return recalibrated; // problem...
    }
