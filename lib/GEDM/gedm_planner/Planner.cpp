...
bool DRAM_ATTR count_load                      = true;
...
void IRAM_ATTR toggle_pwm( bool turn_off, int delay ){ // Not sure if the MCPWM stuff is thread safe but it works
...
bool IRAM_ATTR G_EDM_PLANNER::short_circuit_estop(){
    ...
    if( is_system_op_mode( OP_MODE_EDM_WIRE ) && _state.total_retraction_steps >= rconf.steps_case_0 ){
        ...
        debuglog( planner_messages[PLANNER_ERR_RETRACT_END] );
    }
    //########################################################
    ...
    if( short_circuit_start_time != 0 ){
        if ( esp_timer_get_time() - short_circuit_start_time > plconfig.short_circuit_max_duration_us ){ 
            gconf.edm_pause_motion = true; 
            debuglog( planner_messages[PLANNER_ERR_SHORT] );
        }
    }
    return true;
}
...
void IRAM_ATTR G_EDM_PLANNER::pre_process_history_line_forward( Line_Config &line ){
    reset_rconf();
    get_motion_plan(); 
    if( _state.motion_plan > MOTION_PLAN_HOLD_SOFT && count_load ){
        // flag first contact
...
int IRAM_ATTR G_EDM_PLANNER::get_motion_plan(){
    ...
    if( simulate_gcode ){
        ...
        last_plan          = MOTION_PLAN_FORWARD;
        return 1;
    }

    last_plan          = _state.motion_plan;
    _state.motion_plan = get_calculated_motion_plan();
    ...
    if( _state.motion_plan == MOTION_PLAN_FORWARD || check_broken_wire ){
        //#######################
        ...
        if( 
            ...
            debuglog( planner_messages[PLANNER_ERR_BROKEN_WIRE] );
            gconf.edm_pause_motion = true;
            _state.motion_plan     = MOTION_PLAN_SOFT_SHORT;
            _state.real_short      = 1;
            no_load_steps          = 0;
    ...
    } else if( _state.motion_plan > MOTION_PLAN_FORWARD ){
        //############################
        // Some kind of load going on
        //############################
        // prevent jitter from messing around
        if( 
            _state.motion_plan > MOTION_PLAN_HOLD_SOFT || ( _state.motion_plan == MOTION_PLAN_HOLD_SOFT && ++plan_counter[1] > 2 ) 
        ){ no_load_steps = 0; }
    ...
    if( !is_machine_state( STATE_PROBING ) ){
        if( _state.motion_plan >= MOTION_PLAN_SOFT_SHORT ){
            if( short_circuit_start_time == 0 ){
                short_circuit_start_time = esp_timer_get_time();
            }
            short_circuit_estop_reset_count = 0;
        } else if ( _state.motion_plan <= MOTION_PLAN_HOLD_SOFT && ++short_circuit_estop_reset_count > 4 ){
            short_circuit_start_time        = 0;
            short_circuit_estop_reset_count = 10;
        }
        if( _state.motion_plan == MOTION_PLAN_HARD_SHORT ){
            _state.real_short  = 1;
        } else {
            _state.real_short = 0;
        }
...
void G_EDM_PLANNER::pause( bool redraw ){
    ...
    if( get_quit_motion( true ) || simulate_gcode ){ 
        //toggle_pwm( false, 4 ); // just for the scope
        is_paused = false;
        return; 
    }
    was_paused          = true;
...
void G_EDM_PLANNER::next_gcode_round(){ // function not used at the moment
    // next file pass
    if( gcode_round == 1 ){
        if( is_system_op_mode( OP_MODE_EDM_WIRE ) ){
...
//########################################################################################
// target contain the target positon in mm
...
int G_EDM_PLANNER::get_success_direction( int last_direction ){
    ...
    switch (last_direction){
        ...
        case 1:
            ...
            if( has_reverse >= _state.arc_counter+plconfig.max_reverse_depth ){ //MAX( _state.arc_counter, plconfig.max_reverse_depth ) ){
...
void IRAM_ATTR G_EDM_PLANNER::wire_line_end( Line_Config &line ){
    ...
    //if( arcgen.get_pwm_is_off() ){ toggle_pwm( false, 2 ); }        
    while( true ){
        get_motion_plan();
        if( _state.motion_plan <= MOTION_PLAN_HOLD_SOFT ){ ++pass_counter; } 
        else { pass_counter = 0; }
...
bool IRAM_ATTR G_EDM_PLANNER::process_wire( Line_Config &line ){
    ...
    if( gconf.gedm_retraction_motion ){
        ...
        if( retconf.early_exit && _state.motion_plan <= plconfig.early_exit_on_plan ){ 
            if( ++rconf.early_exit_confirmations > 1 ){ return false; }
        }

        if( rconf.soft_retract_start || rconf.hard_retract_start ){
            line.ignore_feed_limit   = true;
            line.step_delay          = _state.real_short ? process_speeds.WIRE_RETRACT_HARD : process_speeds.WIRE_RETRACT_SOFT;
            if( --rconf.steps_total>0 ){ return true; } 
        }

        if( _state.motion_plan < MOTION_PLAN_HOLD_HARD && !_state.real_short ){
            return false;
        } 
        return true;
...
int32_t IRAM_ATTR G_EDM_PLANNER::get_retraction_steps( float travel_mm ){
    ...
    int travel_steps = round( travel_mm * g_axis[sinker_axis]->steps_per_mm.get() );
    return travel_steps;
}
...
bool G_EDM_PLANNER::flush_end(){
    debuglog("Flushing end", 1 ); // pwm still off
...
bool G_EDM_PLANNER::flush_begin(){
    ...
    debuglog("Flushing begin", 1); // PWM off, save to delay
...
bool IRAM_ATTR G_EDM_PLANNER::move_line( int32_t* target, Line_Config &line ){
    ...
    while( line.step_count < line.step_event_count ){
            ...
            if( line.enable_position_history ){
                ...
                if( gconf.gedm_flushing_motion ){
                    ...
                    if( gconf.gedm_retraction_motion ){ 
                        ...
                        flush_reenable_pwm();
                        int mplan = get_calculated_motion_plan();

                        // back forward
                        if( 
                            flconf.steps_done_forward >= flconf.steps_done_back-flush_offset_steps 
                            || ( mplan >= 4 && flconf.steps_done_back > 2 )
                        ){
                    ...
                    if( gconf.gedm_retraction_motion ){
                        ...
                        ++no_load_steps; // very dirty.. This resets all around the code and is not accurate at all #todo
                        if( _state.total_retraction_steps > 0 ){
                            --_state.total_retraction_steps;
                        }
                    }
        ...
        //##################################################################
        sync_steps = 0; // the number of parallel steps fired: 1 to N_AXIS is possible
        for (int axis = 0; axis < N_AXIS; axis++) {
...
uint8_t IRAM_ATTR G_EDM_PLANNER::plan_history_line( float* target, plan_line_data_t* pl_data ) {
    ...
    if( get_quit_motion() ){
        debuglog("Planner line start aborted");
